...
// Fetch bid notices data from database with pagination
try {
    $pdo = getConnection();
    
    // Get total count
    $countStmt = $pdo->query("SELECT COUNT(*) as total FROM bidnotices");
    ...
    $stmt = $pdo->prepare(
        "SELECT 
            ...
            b.published_date,
            b.closing_date,
            GROUP_CONCAT(bi.image_path ORDER BY bi.id SEPARATOR '\n') AS image_paths
         FROM bidnotices b
         LEFT JOIN bidnotice_images bi ON bi.bidnotice_id = b.id
         GROUP BY b.id, b.description, b.published_date, b.closing_date
...
<head>
    ...
    <title>Bid Notices Management - Admin Panel</title>
...
<body>
    ...
    <main class="main-content">
        ...
        <header class="header">
            <div class="header-left">
                ...
                <h1 class="header-title">Bid Notices Management</h1>
            </div>
        ...
        <div class="table-container">
            <div class="table-header">
                <div class="table-title-section">
                    <h2 class="table-title">Bid Notices</h2>
                    <?php if (!empty($bidnotices)): ?>
                        <span class="table-count">(<?php echo count($bidnotices); ?> items)</span>
                    <?php endif; ?>
                </div>
                <a href="addbidnotice.php" class="btn-add">
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <?php elseif (empty($bidnotices)): ?>
                        <tr>
                            <td colspan="6" style="text-align: center; color: var(--text-secondary); padding: 2rem;">
                                No bid notices found in the database.
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($bidnotices as $bn): ?>
                            <tr id="bidnotice-row-<?php echo htmlspecialchars($bn['id']); ?>">
                                <td><?php echo htmlspecialchars($bn['id']); ?></td>
                                <td><?php echo htmlspecialchars($bn['description']); ?></td>
                                <td>
                                    <?php 
                                        $imagePathsRaw = isset($bn['image_paths']) ? $bn['image_paths'] : '';
                                        $paths = array_filter(array_map('trim', explode("\n", (string)$imagePathsRaw)));
                                    ?>
                                    <?php if (!empty($paths)): ?>
                                        <?php foreach ($paths as $path): ?>
                                            <?php 
                                                ...
                                                $isAbsolute = preg_match('#^(?:https?://|/)#', $path) === 1;
                                                $href = $isAbsolute ? $path : ('../' . ltrim($path, '/'));
                                            ?>
                                            <?php if ($isPdf): ?>
                                                <a href="<?php echo htmlspecialchars($href); ?>" target="_blank" class="file-badge" title="Open PDF">
                                                    <i class="fas fa-file-pdf"></i>
                                                    PDF
                                                </a>
                                            <?php else: ?>
                                                <img src="<?php echo htmlspecialchars($href); ?>" alt="Bid Notice" class="image-thumb" style="margin-right:6px" onerror="this.onerror=null;this.src='https://placehold.co/80x60?text=No+Img';">
                                            <?php endif; ?>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <img src="https://placehold.co/80x60?text=No+Img" alt="No Image" class="image-thumb">
                                    <?php endif; ?>
                                </td>
                                <td><?php echo htmlspecialchars($bn['published_date']); ?></td>
                                <td><?php echo htmlspecialchars($bn['closing_date']); ?></td>
                                <td>
                                    <div class="action-buttons">
                                        <button class="btn-edit" onclick="editBidnotice(<?php echo htmlspecialchars($bn['id']); ?>)">Edit</button>
                                        <button class="btn-delete" onclick="deleteBidnotice(<?php echo htmlspecialchars($bn['id']); ?>)">Delete</button>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
    ...
    <script>
        ...
        // --- BID NOTICE MANAGEMENT FUNCTIONS ---
        function editBidnotice(id) {
            window.location.href = `editbidnotice.php?id=${id}`;
        }

        function deleteBidnotice(id) {
            if (!confirm(`Are you sure you want to delete bid notice with ID: ${id}?`)) {
                return;
            }
            ...
            .then(data => {
                if (data && data.success) {
                    const row = document.getElementById(`bidnotice-row-${id}`);
                    if (row) row.remove();
                    alert('Bid notice deleted successfully.');
                } else {
