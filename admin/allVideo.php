...
// Fetch video data from database with pagination
try {
    $pdo = getConnection();
    
    // Get total count
    $countStmt = $pdo->query("SELECT COUNT(*) as total FROM video");
    ...
    $stmt = $pdo->prepare("SELECT id, name, title, url_path, created_at, updated_at FROM video ORDER BY created_at DESC LIMIT ? OFFSET ?");
...
<body>
    ...
    <main class="main-content">
        ...
        <header class="header">
            <div class="header-left">
                ...
                <h1 class="header-title">Video Management</h1>
            </div>
        ...
        <div class="table-container">
            <div class="table-header">
                <div class="table-title-section">
                    <h2 class="table-title">Videos</h2>
                    <?php if (!empty($videos)): ?>
                        <span class="table-count">(<?php echo count($videos); ?> videos)</span>
                    <?php endif; ?>
                </div>
                <a href="addVideo.php" class="btn-add">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($videos as $video): ?>
                                                         <tr id="video-row-<?php echo htmlspecialchars($video['id']); ?>">
                                 <td><?php echo htmlspecialchars($video['id']); ?></td>
                                 <td><?php echo htmlspecialchars($video['name']); ?></td>
                                <td><?php echo htmlspecialchars($video['title']); ?></td>
                                <td>
                                    <?php if (!empty($video['url_path'])): ?>
                                        <span class="video-url-text"><?php echo htmlspecialchars($video['url_path']); ?></span>
                                    <?php else: ?>
                                        <span class="no-link">No URL</span>
                                    <?php endif; ?>
                                </td>
                                <td><?php echo date('Y-m-d', strtotime($video['created_at'])); ?></td>
                                <td><?php echo date('Y-m-d', strtotime($video['updated_at'])); ?></td>
                                <td>
                                    <div class="action-buttons">
                                        <button class="btn-edit" onclick="editVideo(<?php echo htmlspecialchars($video['id']); ?>)">Edit</button>
                                        <button class="btn-delete" onclick="deleteVideo(<?php echo htmlspecialchars($video['id']); ?>)">Delete</button>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
            ...
            <?php if ($totalPages > 1): ?>
                <div class="pagination-container">
                    <div class="pagination-info">
                        Showing <?php echo $offset + 1; ?> to <?php echo min($offset + $itemsPerPage, $totalItems); ?> of <?php echo $totalItems; ?> videos
                    </div>
    ...
    <script>
        ...
        function editVideo(id) {
            window.location.href = `editVideo.php?id=${id}`;
        }

        function deleteVideo(id) {
            if (!confirm(`Are you sure you want to delete video with ID: ${id}?`)) {
                return;
            }
