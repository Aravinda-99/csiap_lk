...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            ...
            $img_path = 'assets/uploads/newsarticle/' . $filename;

            // Duplicate name check
            $checkStmt = $pdo->prepare('SELECT id FROM article WHERE name = ?');
            $checkStmt->execute([$name]);
            if ($checkStmt->fetch()) {
                if (is_file($destFs)) { @unlink($destFs); }
                $error = 'Article with this name already exists';
            } else {
                $stmt = $pdo->prepare('INSERT INTO article (name, img_path, created_at, updated_at) VALUES (?, ?, NOW(), NOW())');
    ...
    <main class="main-content">
        ...
        <header class="header">
            <div class="header-left">
                ...
                <h1 class="header-title">Add Article</h1>
            </div>
        ...
        <div class="form-container">
            <div class="form-header">
                ...
                <p class="form-subtitle">Fill in the details below to add a new article to the system</p>
            </div>
            ...
            <form method="POST" action="" id="add-article-form" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="image_file">Article Image <span style="color: #dc3545;">*</span></label>
                    <input 
                        type="file" 
                        id="image_file" 
                        name="image_file" 
                        ...
                        required
                    >
                    <div class="help-text">Upload an image (JPG, PNG, GIF, WEBP). Max 8MB. Will be saved to assets/uploads/newsarticle/</div>
                    <div class="error-text" id="file-error">Article image is required</div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        >
                        <div class="help-text">This will be automatically set when the article is added</div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            style="background-color: var(--border-color); cursor: not-allowed;"
                        >
                        <div class="help-text">This will be automatically set when the article is updated</div>
                    </div>
                </div>
