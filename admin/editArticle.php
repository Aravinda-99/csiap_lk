...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $article) {
    ...
    $currentImgPath = $article['img_path'];
    $newImgPath = $currentImgPath;
    ...
    if (empty($name)) {
        ...
        try {
            ...
            if (isset($_FILES['image_file']) && $_FILES['image_file']['error'] === UPLOAD_ERR_OK) {
                $uploadDirFs = __DIR__ . '/../assets/uploads/newsarticle/';
                if (!is_dir($uploadDirFs)) {
                    @mkdir($uploadDirFs, 0755, true);
                }

                $tmpPath = $_FILES['image_file']['tmp_name'];
                $sizeBytes = (int)($_FILES['image_file']['size'] ?? 0);
                ...
                $newImgPath = 'assets/uploads/newsarticle/' . $filename;
            }

            // Duplicate name check (exclude current article)
            $checkStmt = $pdo->prepare('SELECT id FROM article WHERE name = ? AND id != ?');
            $checkStmt->execute([$name, $articleId]);
            if ($checkStmt->fetch()) {
                $error = 'Article with this name already exists';
            } else {
                // Update the article
                $stmt = $pdo->prepare('UPDATE article SET name = ?, img_path = ?, updated_at = NOW() WHERE id = ?');
                $stmt->execute([$name, $newImgPath, $articleId]);

                // If new image was uploaded and different from current, delete old image
                if ($newImgPath !== $currentImgPath && $currentImgPath !== '') {
                    $oldImagePath = __DIR__ . '/../' . $currentImgPath;
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="breadcrumb">
            ...
            <a href="allArticle.php"><i class="fas fa-newspaper"></i> Article Management</a>
            <span class="breadcrumb-separator">/</span>
        ...
        <?php if ($error && !$article): ?>
            ...
            <div class="form-container">
                ...
                <div class="form-buttons">
                    <a href="allArticle.php" class="btn btn-secondary">
        ...
        <?php elseif ($article): ?>
            ...
            <div class="form-container">
                ...
                <form method="POST" action="" id="edit-article-form" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="name">Article Name <span style="color: #dc3545;">*</span></label>
                        <input 
                            type="text" 
                            id="name" 
                            name="name" 
                            value="<?php echo htmlspecialchars($article['name']); ?>"
                            placeholder="Enter article name (e.g., Dinamina 09 Aug 2025, Thinakaran 05 Aug 2025)"
                            required
                            class="<?php echo isset($error) && empty($name ?? '') ? 'invalid' : ''; ?>"
                        >
                        <div class="help-text">Choose a descriptive name for the article</div>
                        <div class="error-text" id="name-error">Article name is required</div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            accept="image/*"
                        >
                        <div class="help-text">Upload a new image (JPG, PNG, GIF, WEBP). Max 8MB. Leave empty to keep current image.</div>
                        <div class="error-text" id="file-error">Please select a valid image file</div>
                        
                        <!-- Current Image Display -->
                        <div class="current-image">
                            <strong>Current Image:</strong><br>
                            <?php if (!empty($article['img_path'])): ?>
                                <img src="../<?php echo htmlspecialchars($article['img_path']); ?>" 
                                     alt="Current Article Image" 
                                     onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                                <span class="no-image" style="display: none;">Image not found</span>
                            <?php else: ?>
                                <span class="no-image">No image currently set</span>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input 
                                type="text" 
                                id="created_at" 
                                value="<?php echo date('Y-m-d H:i:s', strtotime($article['created_at'])); ?>"
                                readonly
                                style="background-color: var(--border-color); cursor: not-allowed;"
                            >
                            <div class="help-text">This was set when the article was first created</div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input 
                                type="text" 
                                id="updated_at" 
                                value="<?php echo date('Y-m-d H:i:s', strtotime($article['updated_at'])); ?>"
                                readonly
                                style="background-color: var(--border-color); cursor: not-allowed;"
                            >
                            <div class="help-text">This will be automatically updated when you save changes</div>
                        </div>
                    </div>
                ...
                fileInput.addEventListener('change', () => {
                    if (fileInput.files && fileInput.files.length > 0) {
                        ...
                        if (!validTypes.includes(file.type)) {
                            fileError.textContent = 'Please select a valid image file (JPG, PNG, GIF, WEBP)';
                            fileError.style.display = 'block';
                        } else if (file.size > 8 * 1024 * 1024) {
                            fileError.textContent = 'Image file must be less than 8MB';
                            fileError.style.display = 'block';
                        } else {
