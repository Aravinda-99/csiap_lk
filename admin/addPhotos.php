...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            ...
            $checkStmt = $pdo->prepare('SELECT id FROM photos WHERE name = ?');
            $checkStmt->execute([$name]);
            if ($checkStmt->fetch()) {
                if (is_file($destFs)) { @unlink($destFs); }
                $error = 'Photo with this name already exists';
            } else {
                $stmt = $pdo->prepare('INSERT INTO photos (name, image_path, status, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())');
                $stmt->execute([$name, $image_path, $status]);
...
<head>
    ...
    <title>Add Photo - Admin Panel</title>
    ...
    <main class="main-content">
        ...
        <header class="header">
            <div class="header-left">
                ...
                <h1 class="header-title">Add Photo</h1>
            </div>
        ...
        <div class="form-container">
            <div class="form-header">
                <h2 class="form-title">Add New Photo</h2>
                <p class="form-subtitle">Fill in the details below to add a new photo to the system</p>
            </div>
            ...
            <form method="POST" action="" id="add-photo-form" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input 
                        ...
                        placeholder="Enter photo name (e.g., 01, Office Opening)"
                        required
                ...
                <div class="form-group">
                    <label for="image_file">Image File <span style="color: #dc3545;">*</span></label>
                    <input 
                        type="file" 
                        id="image_file" 
                        name="image_file" 
                        ...
                        required
                    >
                    <div class="help-text">Upload an image (JPG, PNG, GIF, WEBP). Max 8MB. Will be saved to assets/img/photos/</div>
                    <div class="error-text" id="file-error">Image file is required</div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        >
                        <div class="help-text">This will be automatically set when the photo is added</div>
                    </div>
                </div>
