...
// Fetch vacancies data from database with pagination
try {
    $pdo = getConnection();
    
    // Get total count
    $countStmt = $pdo->query("SELECT COUNT(*) as total FROM vacancies");
    ...
    $stmt = $pdo->prepare(
        "SELECT 
            ...
            v.published_date,
            v.closing_date,
            GROUP_CONCAT(va.advert_path ORDER BY va.id SEPARATOR '\n') AS advert_paths
         FROM vacancies v
         LEFT JOIN vacancy_adverts va ON va.vacancy_id = v.id
         GROUP BY v.id, v.description, v.published_date, v.closing_date
...
<head>
    ...
    <title>Vacancies Management - Admin Panel</title>
...
<body>
    ...
    <main class="main-content">
        ...
        <header class="header">
            <div class="header-left">
                ...
                <h1 class="header-title">Vacancies Management</h1>
            </div>
        ...
        <div class="table-container">
            <div class="table-header">
                <div class="table-title-section">
                    <h2 class="table-title">Vacancies</h2>
                    <?php if (!empty($vacancies)): ?>
                        <span class="table-count">(<?php echo count($vacancies); ?> items)</span>
                    <?php endif; ?>
                </div>
                    ...
                    <?php elseif (empty($vacancies)): ?>
                        <tr>
                            <td colspan="6" style="text-align: center; color: var(--text-secondary); padding: 2rem;">
                                No vacancies found in the database.
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($vacancies as $vc): ?>
                            <tr id="vacancy-row-<?php echo htmlspecialchars($vc['id']); ?>">
                                <td><?php echo htmlspecialchars($vc['id']); ?></td>
                                <td><?php echo htmlspecialchars($vc['description']); ?></td>
                                <td>
                                    <?php 
                                        $advertsRaw = isset($vc['advert_paths']) ? $vc['advert_paths'] : '';
                                ...
                                <td><?php echo htmlspecialchars($vc['published_date']); ?></td>
                                <td><?php echo htmlspecialchars($vc['closing_date']); ?></td>
                                <td>
                                    <div class="action-buttons">
                                        <button class="btn-edit" onclick="editVacancy(<?php echo htmlspecialchars($vc['id']); ?>)">Edit</button>
                                        <button class="btn-delete" onclick="deleteVacancy(<?php echo htmlspecialchars($vc['id']); ?>)">Delete</button>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
    ...
    <script>
        ...
        function editVacancy(id) {
            window.location.href = `editvacancy.php?id=${id}`;
        }

        function deleteVacancy(id) {
            if (!confirm(`Are you sure you want to delete vacancy with ID: ${id}?`)) {
                return;
            }
            ...
            .then(data => {
                if (data && data.success) {
                    const row = document.getElementById(`vacancy-row-${id}`);
                    if (row) row.remove();
