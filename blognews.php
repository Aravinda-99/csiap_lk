...
<html>
    ...
    <body>
        ...
        // $json = file_get_contents('https://csiaplk.blogspot.com/feeds/posts/default?alt=json&max-results=3');
	    ...
	    ?>
                <div class="accordion" id="accordionExample<?php echo($loopcount); ?>">
	                <div class="card">
	                    ...
	                    <p>Posted by <?php echo($value->{'author'}[0]->{'name'}->{'$t'}) ; ?><a target="-_blank" href="<?php echo($value->{'link'}['2']->{'href'}) ; ?>" class="btn btn-info btn-sm ml-3"><i class="fas fa-link"></i> Read full article</a> <span class="float-right">Published Date : <?php echo substr(nl2br($value->{'published'}->{'$t'}),0,10) ; ?></span></p>
	                    </div>
	                    </div>
	                </div>
                </div>
